...
/// 打开工作日志面板窗口
#[tauri::command]
pub async fn open_todo_panel(
    main_window: WebviewWindow,
    app_handle: tauri::AppHandle,
) -> Result<(), String> {
    let todo_panel = match app_handle.get_webview_window("todo-panel") {
        Some(window) => window,
        None => return Err("Todo panel window not found".to_string()),
    };
    ...
    let todo_x: f64 = if can_fit_right {
        right_x
    } else if can_fit_left {
        left_x
    } else {
        let right_visible = screen_width - right_x;
        let left_visible = left_x + panel_width;
        if right_visible >= left_visible { right_x } else { left_x }
    };

    let mut todo_y: f64 = main_pos_logical.y;
    if todo_y + panel_height > screen_height {
        todo_y = screen_height - panel_height;
    }
    if todo_y < 0.0 {
        todo_y = 0.0;
    }
    ...
    todo_panel
        ...
        .map_err(|e| format!("Failed to set todo panel position: {}", e))?;

    todo_panel.show().map_err(|e| format!("Failed to show todo panel: {}", e))?;
    todo_panel.set_focus().map_err(|e| format!("Failed to focus todo panel: {}", e))?;
...
/// 关闭工作日志面板窗口
#[tauri::command]
pub async fn close_todo_panel(app_handle: tauri::AppHandle) -> Result<(), String> {
    if let Some(window) = app_handle.get_webview_window("todo-panel") {
        window.hide().map_err(|e| format!("Failed to hide todo panel: {}", e))?;
    }
    Ok(())
}
